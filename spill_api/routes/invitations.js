...
router.get('/user', function(req, res, next) {
	connection.query('SELECT * from Invitation WHERE invitationFromUserId = ?',req.query.s, function (error, results, fields) {
		if(error){
			res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
			//If there is error, we send the error in the error section with 500 status
		} else {
...
router.get('/invitedUser', function(req, res, next) {
	connection.query('SELECT * from Invitation WHERE invitedUserId = ?',req.query.s, function (error, results, fields) {
		if(error){
			res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
			//If there is error, we send the error in the error section with 500 status
		} else {
